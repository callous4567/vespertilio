...
/*
Call this to initalize the provided sram_t object spi-wise. 
*/
void SRAM_init(sram_t *SRAM) {
    ...
    SRAM_SPI8(SRAM);
    SRAM_sequential_mode(SRAM); // set the default mode to sequential just because this is all we are using in this project. 
...
/*
Call this appropriately to configure the SRAM object DMA-wise. 
You need to provide pointers for the malloc'd buffers A and B appropriately, alongside their size.
*/
void SRAM_dma_init(sram_t *SRAM, uint16_t *BUF_A, uint16_t *BUF_B, int *BUF_SIZE) {

    // Configure DMA channel from ADC to the SRAM
    SRAM->adc_sram_chan = dma_claim_unused_channel(true);
    SRAM->adc_sram_conf = dma_channel_get_default_config(SRAM->adc_sram_chan);
    channel_config_set_transfer_data_size(&SRAM->adc_sram_conf, DMA_SIZE_16);
    channel_config_set_read_increment(&SRAM->adc_sram_conf, false);
    channel_config_set_write_increment(&SRAM->adc_sram_conf, false);
    channel_config_set_dreq(&SRAM->adc_sram_conf, DREQ_ADC);
    ...
    SRAM->sram_a_chan = dma_claim_unused_channel(true);
    SRAM->sram_a_conf = dma_channel_get_default_config(SRAM->sram_a_chan);
    channel_config_set_transfer_data_size(&SRAM->sram_a_conf, DMA_SIZE_16);
    channel_config_set_read_increment(&SRAM->sram_a_conf, false);
    channel_config_set_write_increment(&SRAM->sram_a_conf, true);
    dma_channel_configure(
        SRAM->sram_a_chan,
        &SRAM->sram_a_conf,
        BUF_A,
    ...
    // Configure DMA channel from the SRAM to buffer B 
    SRAM->sram_b_chan = dma_claim_unused_channel(true);
    SRAM->sram_b_conf = dma_channel_get_default_config(SRAM->sram_b_chan);
    channel_config_set_transfer_data_size(&SRAM->sram_b_conf, DMA_SIZE_16);
    channel_config_set_read_increment(&SRAM->sram_b_conf, false);
    channel_config_set_write_increment(&SRAM->sram_b_conf, true);
    dma_channel_configure(
        SRAM->sram_b_chan,
        &SRAM->sram_b_conf,
        BUF_A,
...
// Get the hw instance of the corresponding SRAM for the spi 
